...
local util = require("neotest-jest-loggi-beyond.util")
...
local adapter = { name = "neotest-jest-loggi-beyond" }


local function is_in_loggi_beyond_ui(file_path)
  if string.match(file_path, "targets/beyond") then
    return true
  end
...
function adapter.is_test_file(file_path)
  ...
  for _, ext in ipairs({ "js", "jsx", "ts", "tsx" }) do
    if string.match(file_path, "%." .. "test" .. "%." .. ext .. "$") then
      is_test_file = true
      goto matched_pattern
    end
  end
  ::matched_pattern::
  return is_test_file and is_in_loggi_beyond_ui(file_path)
end
